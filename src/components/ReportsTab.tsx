...
export function ReportsTab(props: IProps) {
  const { projectPlans, planColumn, floatTop, step, orgSteps, sectionArr } =
    props;

  const { pasId } = useParams();
  const t: IReportsTabStrings = useSelector(transcriptionTabSelector);
  ...
  const columnDefs = [
    ...
    { name: 'report', title: t.action },
    { name: 'passages', title: ts.passages },
    { name: 'action', title: '\u00A0' },
  ];
  const columnWidths = [
    { name: 'name', width: 300 },
    { name: 'report', width: 200 },
    { name: 'passages', width: 100 },
  ...
  return (
    <Box id="ReportsTab" sx={{ display: 'flex' }}>
      <div>
        <TabAppBar
          position="fixed"
